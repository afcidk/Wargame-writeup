...
def _decrypt(vec: List[int], key: List[int]):
    ...
    for i in range(32, 0, -1):
        ...
        for t in range(i):
            sum = sum + toyota & mask
        prev0 = ((((vec[0] << 4) + key[2]) & mask) ^ ((vec[0] + sum) & mask) ^ (((vec[0] >> 5) + key[3]) & mask))
        vec[1] = (vec[1] - prev0) & mask
        prev1 = ((((vec[1] << 4) + key[0]) & mask) ^ ((vec[1] + sum) & mask) ^ (((vec[1] >> 5) + key[1]) & mask))
...
def decrypt(ciphertext: bytes, key: bytes):
    ...
    for idx in range(0, len(ciphertext), 8):
        plaintext += 逆轉換(_decrypt(正轉換(ciphertext[idx:idx+8]), 正轉換(key)))
    return plaintext

# Solution: 1599977586 b'FLAG{4lq7mWGh93}'
if __name__ == '__main__':
    # This ciphertext is copied manually from output.txt
    ciphertext = b'\x77\xf9\x05\xc3\x9e\x36\xb5\xeb\x0d\xee\xcb\xb4\xeb\x08\xe8\xcb'
