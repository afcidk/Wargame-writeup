...
int cur_ind = 1;
int gen_new(char *buf, int level) {
    if (level == 3) {
        unsigned int result = crc32(0, buf, cur_ind*3+3);
        //printf("check buf: %s, crc32 = 0x%x\n", buf, result);
        return result == hashes[cur_ind];
    }

    for (int i=0; i<strlen(printable); ++i) {
        buf[cur_ind*3+level] = printable[i];
        if (gen_new(buf, level+1)) {
            return 1;
        }
        buf[cur_ind*3+level] = 0;
    }
    return 0;
}
