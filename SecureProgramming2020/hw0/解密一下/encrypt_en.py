...
def _encrypt(vec: List[int], key: List[int]):
    ...
    for _ in range(32):
        sum = sum + toyota & mask
        vec[0] = vec[0] + ((((vec[1] << 4) + key[0]) & mask) ^
                           ((vec[1] + sum) & mask) ^
                           (((vec[1] >> 5) + key[1]) & mask)) & mask
        vec[1] = vec[1] + ((((vec[0] << 4) + key[2]) & mask) ^
                           ((vec[0] + sum) & mask) ^
                           (((vec[0] >> 5) + key[3]) & mask)) & mask
    return vec
...
def encrypt(plaintext: bytes, key: bytes):
    ...
    for idx in range(0, len(plaintext), 8):
        ciphertext += negative(
            _encrypt(positive(plaintext[idx:idx + 8]), positive(key)))
    return ciphertext
